...
*4. Search & Filter*
  - Tips: Use org name, contact name, or ID.
  - **Filtering**: Add comma-separated filters, e.g., \`/searchdeals ProjectX filter:owner:glenn,status:Open\`
...
Need help? Just type /help!
  ...
  bot.onText(/\/login/, async (msg) => {
    ...
    await bot.sendMessage(
      chatId,
      "Please enter the alias for your CRM instance (e.g., 'mycompany', 'anothercrm'):"
    );
  });
  ...
  bot.onText(/\/listcrms/, async (msg) => {
    ...
    if (crmAliases.length === 0) {
      return bot.sendMessage(
        chatId,
        "No CRMs configured yet. Use `/login` to add one."
      );
    }
    const crmList = crmAliases
      .map((alias) => {
  ...
  bot.onText(/\/usecrm (.+)/, async (msg, match) => {
    ...
    if (crmConfig && crmManager.setActiveCrmAlias(chatId, alias)) {
      ...
      await bot.sendMessage(
        chatId,
        `CRM with alias '${alias}' not found or not configured.`
      );
    }
  });
    ...
    switch (loginState) {
      case LOGIN_STATE.WAITING_FOR_ALIAS:
        ...
        if (!crmManager.validateCrmAlias(alias)) {
          await bot.sendMessage(
            chatId,
            `Alias '${alias}' not recognized. Please contact your admin or try another alias.`
          );
        ...
        await bot.sendMessage(
          chatId,
          `Alias '${alias}' found. Now, please enter your CRM username:`
        );
        break;
      ...
      case LOGIN_STATE.WAITING_FOR_USERNAME:
        ...
        await bot.sendMessage(
          chatId,
          `Username '${username}' recorded. Please enter your CRM password:`
        );
        break;
      ...
      case LOGIN_STATE.WAITING_FOR_PASSWORD:
        ...
        try {
          const isAuthenticated = await crmManager.authenticateUser(
            loginAlias,
            loginUsername,
            password
          );
          if (isAuthenticated) {
            ...
            if (crmDetails) {
              await crmManager.addAuthenticatedCrm(
                chatId,
                loginAlias,
                crmDetails.url
              );
              ...
              await bot.sendMessage(
                chatId,
                `Successfully logged in to CRM '${loginAlias}'! It is now your active CRM.`
              );
            } else {
              await bot.sendMessage(
                chatId,
                `Login successful, but could not retrieve CRM details for '${loginAlias}'. Please contact admin.`
              );
            }
          } else {
            await bot.sendMessage(
  ...
  bot.onText(/\/createtask (\S+) (\S+)(?: (.*))?/s, async (msg, match) => {
    ...
    if (!activeCrm) {
      return bot.sendMessage(
        chatId,
        "No active CRM selected or authenticated. Use `/login` to set up your CRM."
      );
    }
    ...
    try {
      ...
      await bot.sendMessage(
        chatId,
        `Task creation request sent for '${docName.trim()}'.`
      );
    } catch (error) {
      console.error(
        "[CREATE_TASK] ERROR:",
      ...
      await bot.sendMessage(
        chatId,
        "Failed to create task. Please check n8n webhook URL and CRM details."
      );
    }
  });
  ...
  bot.onText(/\/createnote (\S+) "([^"]+)"(?: (.*))?/s, async (msg, match) => {
    ...
    try {
      ...
      await bot.sendMessage(
        chatId,
        `Note creation request sent for '${docName.trim()}'.`
      );
    } catch (error) {
      console.error(
        "[CREATE_NOTE] ERROR:",
      ...
      await bot.sendMessage(
        chatId,
        "Failed to create note. Please check n8n webhook URL and CRM details."
      );
    }
  });
...
async function runSearch(chatId, input, doctype) {
  ...
  if (!query) {
    return bot.sendMessage(
      chatId,
      `Use \`/search${currentDoctype === "CRM Lead" ? "leads" : "deals"} <term>\` first.`
    );
  }

  try {
    let fieldsToFetch = ["name", "organization", "status", "owner", "modified"];
    ...
    console.log(
      `[SEARCH] Doctype: ${currentDoctype}, Query: ${query}, Filters: ${JSON.stringify(filters)}, Page: ${page}`
    );
    ...
    const orgRes = await axios.get(
      `${crmBaseUrl}/api/resource/${currentDoctype}`,
      {
        params: {
    ...
    const nameSearchField =
      currentDoctype === "CRM Deal" ? "name" : "first_name";
    ...
    if (!combined.length) {
      return bot.sendMessage(
        chatId,
        `No ${currentDoctype.toLowerCase()}s found for "${query}"`
      );
    }
    ...
    if (currentDoctype === "CRM Lead") {
      ...
      combined.forEach((item, i) => {
        keyboard.push([
          {
            text: `Update ${i + 1 + start}`,
            callback_data: `update_lead:${item.name}`,
          },
          {
            text: `Convert ${i + 1 + start} -> Deal`,
            callback_data: `convert_lead:${item.name}`,
          },
        ]);
      });
    } else if (currentDoctype === "CRM Deal") {
      ...
      combined.forEach((item, i) => {
        keyboard.push([
          {
            text: `Update ${i + 1 + start}`,
            callback_data: `select_deal:${item.name}`,
          },
        ]);
      });
    }
    ...
    if (page > 1)
      nav.push({
        text: "Previous",
        callback_data: `prev_${currentDoctype === "CRM Lead" ? "lead" : "deal"}`,
      });
    if (combined.length === 5)
      nav.push({
        text: "More",
        callback_data: `more_${currentDoctype === "CRM Lead" ? "lead" : "deal"}`,
      });
    nav.push({
      text: "Filter",
      callback_data: `filter_${currentDoctype === "CRM Lead" ? "lead" : "deal"}`,
    });
    nav.push({
      text: "Create new",
      callback_data: `creat_${currentDoctype === "CRM Lead" ? "lead" : "deal"}`,
    });
    keyboard.push(nav);
    // ===================================================
    ...
    console.error(
      `[SEARCH] ${currentDoctype} ERROR:`,
      err.response?.data || err.message
    );
    bot.sendMessage(
      chatId,
      `${currentDoctype} search failed. Check CRM URL or API key.`
    );
  }
}
